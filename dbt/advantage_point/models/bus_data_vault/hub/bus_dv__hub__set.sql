...
records_rownum as (
    select
        hub.*,
        row_number() over (partition by hub.hk_set order by hub_rec_src.sort_order) as rn -- assing row number
    from records_hkey as hub
    left join hub_record_sources as hub_rec_src on 1=1
        and hub_rec_src.hub_name = 'bus_dv__hub__set'
...
final as (
    ...
    where 1=1
        ...
        and not exists (
            ...
            where 1=1
                and existing.hk_set = incr.hk_set
